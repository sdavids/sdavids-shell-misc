...
readonly easyrsa_key="${pki_dir}/private/${host_name}.key"
readonly easyrsa_cert="${pki_dir}/issued/${host_name}.crt"
...
if [ ! -e "${cert_path}" ] && [ -f "${easyrsa_cert}" ]; then
  cp "${easyrsa_cert}" "${cert_path}"
fi

if [ ! -e "${key_path}" ] && [ -f "${easyrsa_key}" ]; then
  cp "${easyrsa_key}" "${key_path}"
fi
