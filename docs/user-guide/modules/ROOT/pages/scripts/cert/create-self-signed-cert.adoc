...
`d` :: the directory (`$PWD` if not given) the key and certificate will be created in; will be created if it does not exit yet
...
[IMPORTANT]
====
Ensure that the common name (set via `-c`) of the generated certificate has an entry in `/etc/hosts`.
