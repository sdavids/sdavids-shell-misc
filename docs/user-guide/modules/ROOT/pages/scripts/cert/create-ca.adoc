...
[IMPORTANT]
====
When the CA root certificate <<create-ca-expire,expires>>, the entire CA, xref:scripts/cert/copy-ca-root-cert.adoc[copied] root certificates, and all xref:scripts/cert/create-ca-based-cert.adoc[created] and copied certificates become invalid.
